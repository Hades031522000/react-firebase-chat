...
if(isset($_POST['form1'])) {
    ...
    if($path == '') {
        ...
        if( $ext!='jpg' && $ext!='png' && $ext!='jpeg' && $ext!='gif' && $ext!='webp' && $ext!='svg' ) {
            $valid = 0;
            $error_message .= 'You must have to upload jpg, jpeg, gif, webp, svg or png file<br>';
        }
    }
 ...
 //   $stripe_secret_key               = $row['stripe_secret_key'];
  ...
  //  $home_testimonial_on_off         = $row['home_testimonial_on_off'];
   // $home_blog_on_off                = $row['home_blog_on_off'];
    ...
    // 6 lines bollow mustbe comments
